...
| MCP 클라이언트 (선택) | 본인이 쓰는 AI 코딩 도구에 `@daven/mcp` 연결 |
...
| `@daven/mcp` | MCP 서버. 개발할 때 AI 도구가 Daven 기능을 호출합니다. |
...
<Steps>
  ...
  <Step title="MCP 클라이언트에 연결 (선택)">
    본인이 쓰는 AI 코딩 플랫폼의 MCP 설정에 `@daven/mcp`를 추가합니다. Settings → Developer / MCP에 있는 설정 예시를 참고하세요.
    ...
    플랫폼마다 설정 UI·파일 경로는 다를 수 있습니다. 연결 후 에이전트가 계정·견적·미디어 도구로 구현을 도울 수 있습니다. 앱 작성 규칙은 MCP의 `daven_guidelines`를 따르면 됩니다.
  </Step>
</Steps>
