...
교육앱을 Daven에 올리는 데 **당신이 하는 일**만 정리합니다. 교실에 앱을 연결·실행하는 방식은 Daven이 처리합니다.
...
| MCP 키 | `dvn_mcp_live_…` 개발자 시크릿. Cursor·서버 측 호출·Playground 검증에 씁니다. 브라우저에 넣지 마세요. |
...
| `@daven/mcp` | Cursor/Claude용 MCP 서버. 개발할 때 AI가 Daven 도구를 호출합니다. |

<Warning>
  MCP를 Vercel에 설치하지 않습니다. Vercel에는 교육앱을 배포하고, 서버 환경 변수에만 MCP 키를 둡니다. MCP 서버는 Cursor(또는 동등한 IDE)에 연결합니다.
</Warning>
...
<Steps>
  ...
  <Step title="Cursor에 MCP 연결 (선택)">
    바이브코딩할 때 Settings에 있는 Cursor 스니펫을 붙여 넣습니다.
    ...
    에이전트가 계정·견적·미디어 생성 도구로 구현을 돕습니다. 앱 작성 규칙은 MCP의 `daven_guidelines`를 따르면 됩니다.
  </Step>
</Steps>
...
| 단계 | 통과 조건 |
| --- | --- |
| 키 | Settings에서 발급, 클라이언트에 노출되지 않음 |
