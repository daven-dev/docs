...
| MCP key | Developer secret `dvn_mcp_live_…`. Used for Cursor, server-side calls, and Playground. Never put it in the browser. |
...
| `@daven/mcp` | MCP server for Cursor/Claude so the agent can call Daven tools while you build |

<Warning>
  You do not install MCP on Vercel. Deploy the education app on Vercel and keep the MCP key in **server** env vars. Connect the MCP server in Cursor (or an equivalent IDE).
</Warning>
...
<Steps>
  ...
  <Step title="Connect Cursor MCP (optional)">
    For vibe-coding, paste the Cursor snippet from Settings.
    ...
    The agent can use account, quote, and media tools while you build. App-writing rules live in the MCP tool `daven_guidelines`.
  </Step>
</Steps>
